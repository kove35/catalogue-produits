...
public class CatalogueMetierImpl implements IcatalogueMetier{
	...
	public void addProduit(Produit p) 
	{
		...
		try{
			...
			PreparedStatement ps=conn.prepareStatement("insert into PRODUIT(REF_PROD,DESIGNATION,PRIX,QUANTITE) VALUE (?,?,?,?)");
	...
	public Produit getProduit(String ref) {
		...
		Connection conn=SingletonConnection.getConnection();
		try{
					PreparedStatement ps=conn.prepareStatement("select * from PRODUIT where REF_PROD like ?");
					ps.setString(1, ref);
					ResultSet rs=ps.executeQuery();
					if(rs.next())
						{	p=new Produit();
	...
	public void updateProduit(Produit p) 
	{
		...
		try{
			...
			PreparedStatement ps=conn.prepareStatement("update from PRODUIT set DESIGNATION=?,PRIX=?,QUANTITE=? where REF_PROD=?");
